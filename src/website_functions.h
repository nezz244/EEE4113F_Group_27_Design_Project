#ifndef WEBSITE_FUNCTIONS
#define WEBSITE_FUNCTIONS
...
bool checkForBluetooth(){
    ...
    if (count > 0) {
        ...
        // Serial.printf("%d: %s\n", i+1, device.toString().c_str());
        // }
        Serial.printf("%d bluetooth devices detected!", count);
        BLEDevice::deinit();
        return true;
    } 
    else {
...
void append_page_header() {
  ...
  webpage += F("<head>");
  webpage += F("<title>Smart Camera Trap</title>"); // NOTE: 1em = 16px
  ...
  webpage += F("</style></head><body><h1>Smart Camera Trap</h1>");
  webpage += F("<ul>");
...
{ 
  ...
  if(uploadfile.status == UPLOAD_FILE_START)
  {
    ...
    fs::FS &fs = SD_MMC;
    File root = fs.open(filename, FILE_WRITE);
...
void hostNetwork(){
  ...
  unsigned long loopTime = 60000; //1 minute
  
  if (checkForBluetooth()){
    Serial.println("found bluetooth devices!");
    WiFi.softAP("SCT2", "12345678"); //Network and password for the access point genereted by ESP32
    ...
    server.on("/",         SD_dir);
    // server.on("/download", download_all);
  ...
  // });
    ...
    startTime = millis();
    endTime = millis();
    while ((((endTime = millis())- startTime) < loopTime) || (WiFi.softAPgetStationNum() > 0)){
      // while (true){

      
      // Serial.println("reached position 2");
    while ((((endTime = millis())- startTime) < loopTime)){
      server.handleClient();
      // delay(1000);
      // prevent endTime from overflowing when connected for a long time
    ...
    Serial.println("No bluetooth devices detected :(");
  }
  Serial.println("reached position 4");
  MDNS.end();
  Serial.println("reached position 5");
  // return;
}
